/**
 * rp2040_sio.cs — PicoDesk-patched copy of
 * external/renode-rp2040/emulation/peripherals/sio/rp2040_sio.cs.
 *
 * Upstream bug: spinlock ownership is stored as the CPU slot index, and slot
 * 0 collides with the "free" sentinel — core 0 can never actually hold a
 * lock, so both cores enter FreeRTOS SMP critical sections simultaneously
 * and the kernel asserts in vPortRecursiveLock. This copy stores owner as
 * slot+1 and models the real try-claim semantics (a read of a HELD lock
 * returns 0, even for the holder). Loaded by
 * sim/initialize_peripherals_patched.resc in place of the upstream file.
 *
                ...
                if (r >= Registers.SPINLOCK_0 && r <= Registers.SPINLOCK_31)
                {
                    ...
                    r.Define(this)
                        .WithValueField(0, 32, FieldMode.Write | FieldMode.Read,
                            writeCallback: (_, value) =>
                            {
                                // Release: any write by the holder frees the
                                // lock (owner stored as slot + 1, so slot 0 is
                            ...
                            valueProviderCallback: _ =>
                            {
                                // Try-claim: only a free lock can be taken;
                                // reading a held lock returns 0 for everyone,
                                // holder included (real RP2040 semantics).
                                var cpu = CurrentCpu() + 1;
                                if (spinlocks[id] == 0)
                                {
                                    spinlocks[id] = cpu;
